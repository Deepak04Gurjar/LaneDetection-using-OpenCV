...
def region_of_interest(image):  #Narrow Down our area of intrest
    height=image.shape[0]      #height of canny image
    polygon=np.array([
    [(300,height),(1100,height),(550,250)]     #To create a mask
    ])
    mask=np.zeros_like(image)       #mask
    cv2.fillPoly(mask,polygon,255)
    masked_image=cv2.bitwise_and(image,mask) #Bitwise and between mask and canny image
    return masked_image

# image=cv2.imread('test5.webp')
...
cap=cv2.VideoCapture("testV_3.mp4")
while(cap.isOpened()):
    ret,frame=cap.read()
    if not ret:
        break
